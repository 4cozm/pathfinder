...
// Dart Sass (require("sass")) only — node-sass 제거로 Windows에서 VS 빌드 불필요
// sass.compiler = require("node-sass");  // deprecated, use line 19 compiler
...
try {
  ...
  try {
    ...
    printError(err.message, 'Missing "PATHFINDER.VERSION" in "' + pathfinderConfigFileApp + '"');
    process.exit(1);
  }
} catch (err) {
  printError(err.message, 'Check read permissions for "' + pathfinderConfigFileApp + '"');
  process.exit(1);
}
...
colors.theme({
  ...
  divider: (text) =>
    colors.disabled(
      `${(text || "").trim().length ? `═ ${(text || "").trim().toUpperCase()} ═` : ""}`.padEnd(cliBoxLength, "═"),
    ),
});
  ...
  for (let confKey in confUser) {
    if (confUser.hasOwnProperty(confKey)) {
      if (confDefault.hasOwnProperty(confKey)) {
        if (typeof confUser[confKey] === "object" && typeof confDefault[confKey] === "object") {
...
let printJsSummary = () => {
  ...
  let sortCol =
    CONF.JS.BROTLI || CONF.CSS.BROTLI || CONF.JS.GZIP || CONF.CSS.GZIP || CONF.JS.UGLIFY
      ? 10
      : CONF.JS.SOURCEMAPS
        ? 3
        : 1;
  let refAllCol =
    CONF.JS.BROTLI || CONF.CSS.BROTLI
      ? 9
      : CONF.JS.GZIP || CONF.CSS.GZIP
        ? 6
        : CONF.JS.UGLIFY
          ? 3
          : CONF.JS.SOURCEMAPS
    ...
    arr.map((x, rowIdx) =>
      x.map((y, i) => {
        if (mapping.byteCols.includes(i)) {
          ...
          let isSuccess = y === 1 && !CONF.JS.UGLIFY && tableHead[i] === "src_percent";
  ...
  for (let i = 0; i < numFormatCols.length; i++) {
    table.attrRange(
      { row: [0], column: [numFormatCols[i], numFormatCols[i] + 1] },
      {
        align: "right",
        rightPadding: 1,
      },
    ...
    if (highLightRow.hasOwnProperty(highLight)) {
      for (let highLightSection in highLightRow[highLight]) {
        for (let i = 0; i < highLightRow[highLight][highLightSection].length; i++) {
          ...
          table.attrRange(
            { row: [rowIdx + 1, rowIdx + 2], column: [colFrom, colTo] },
            {
              color: color,
            },
          );
...
gulp.task("task:hintJS", () => {
  return gulp
    .src([PATH.JS.SRC, "!" + PATH.JS.SRC_LIBS])
...
gulp.task("task:diffJS", () => {
  return gulp
    .src(PATH.JS.SRC, { base: "js", since: gulp.lastRun("task:diffJS") })
    .pipe(
      filter((file) => {
...
let getAssetFilterOptions = (compression, fileExt) => {
  return {
    srcModules: [PATH.ASSETS.DEST + "/**/*." + fileExt, "!" + PATH.ASSETS.DEST + "/js/" + CONF.TAG + "{,/**/*}"],
...
let gzipAssets = (config, taskName) => {
  return gulp
    .src(config.srcModules, { base: "public", since: gulp.lastRun(taskName) })
...
gulp.task("task:renameJsDest", () => {
  ...
  return gulp
    .src(PATH.JS.DEST_BUILD + "/**/*.{" + fileExt.join(",") + "}", {
      base: PATH.JS.DEST_BUILD,
      since: gulp.lastRun("task:renameJsDest"),
    })
...
let imgWebpHandler = (config, taskName) => {
  return gulp
    .src(config.src, { base: config.base, since: gulp.lastRun(taskName) })
...
let imgResizeHandler = (config, taskName) => {
  return gulp
    .src(config.src, { base: "img", since: gulp.lastRun(taskName) })
...
gulp.task("task:imgHeadResize", gulp.series(gulp.parallel(...imgResizeHeaderTasks), "task:imgHeadToWEBP"));

gulp.task("task:imgGalleryCopy", () => {
  return gulp
    .src(PATH.IMG.SRC_GALLERY, { base: "img", since: gulp.lastRun("task:imgGalleryCopy") })
    .pipe(gulp.dest(PATH.ASSETS.DEST + "/img/" + CONF.TAG));
});

gulp.task("task:imgGallery", gulp.series("task:imgGalleryCopy", "task:imgGalleryToWEBP"));

gulp.task("task:imgRest", () => {
  return gulp
    .src([PATH.IMG.SRC, `!${PATH.IMG.SRC_HEADER}`, `!${PATH.IMG.SRC_GALLERY}`, `!${PATH.IMG.SRC_SVG}`], {
      base: "img",
      since: gulp.lastRun("task:imgRest"),
    })
...
gulp.task("task:imgSVG", () => {
  return gulp
    .src(PATH.IMG.SRC_SVG, { base: "img", since: gulp.lastRun("task:imgSVG") })
...
gulp.task("task:printConfig", (done) => {
  ...
  for (let key in configFlat) {
    if (configFlat.hasOwnProperty(key)) {
      ...
      log(colors.yellow(padEnd(key, columnLength)), configFlat[key] ? success(value) : error(value));
    }
  }
  log(colors.divider(" "));
  done();
...
gulp.task("task:checkConfig", (done) => {
  if (!CONF.TAG) {
    printError(
      `Missing TAG version. Add param: ${colors.yellow("--tag")}`,
      `$ npm run gulp default -- --tag="${tagVersion}"`,
    );
    process.exit(0);
  }
  done();
...
gulp.task(
  ...
  gulp.series(
    (done) => {
      ...
      CONF = mergeConf(
        ["TASK", "TAG", "IMG"].reduce((obj, key) => ({ ...obj, [key]: CONF[key] }), {}),
        CONF_IMAGES,
      );
      done();
    },
...
gulp.task(
  "task:updateJsDest",
  gulp.series(
    gulp.parallel("task:gzipJsAssets", "task:brotliJsAssets"),
    "task:renameJsDest",
    // 'task:printJsSummary',
 ...
 */
gulp.task("task:buildJs", gulp.series("task:concatJS", "task:diffJS", "task:cleanJsDest", "task:updateJsDest"));

/**
 * build SCSS dest files
 */
gulp.task("task:buildCss", gulp.series("task:sass"));

/**
 * build IMG dest files
 */
gulp.task("task:buildImg", gulp.parallel("task:imgHeadResize", "task:imgGallery", "task:imgRest", "task:imgSVG"));
 ...
 */
gulp.task("task:watchJsSrc", gulp.series("task:hintJS", "task:diffJS", "task:updateJsDest", "task:notifyJsDone"));
...
gulp.task(
  ...
  gulp.series(
    "task:buildCss",
    // 'task:cleanCss',
    gulp.parallel("task:gzipCssAssets", "task:brotliCssAssets"),
    // 'task:printJsSummary',
    "task:notifyCssDone",
  ),
);

gulp.task("task:watchImg", gulp.series("task:buildImg"));
...
gulp.task("task:setWatcher", gulp.parallel("task:setWatcherJs", "task:setWatcherCss", "task:setWatcherImg"));

// == Default/Main tasks ==============================================================================================

gulp.task("help", gulp.series("task:printHelp"));
...
gulp.task(
  ...
  gulp.series(
    "task:configDevelop",
    gulp.parallel(
      gulp.series("task:cleanJsDestBuild", "task:watchJsSrc"),
      gulp.series("task:cleanCssDest", "task:watchCss"),
      gulp.series("task:cleanImgDest", "task:watchImg"),
    ),
    "task:printJsSummary",
    "task:setWatcher",
  ),
...
gulp.task(
  ...
  gulp.series(
    "task:configProduction",
    gulp.parallel(
      gulp.series("task:cleanJsDestBuild", "task:buildJs"),
      gulp.series("task:cleanCssDest", "task:watchCss"),
      gulp.series("task:cleanImgDest", "task:watchImg"),
    ),
    "task:printJsSummary",
...
/**
 * Production build without image tasks (Windows 등에서 gm/이미지 빌드 생략)
 */
gulp.task(
  "production-noimg",
  gulp.series(
