...
- **Agent debug telemetry (temporary):** During incident debugging, map frontend code can emit structured runtime events for `getAccessData`, WebSocket `connect/error/close`, and Ajax shutdown handlers to a local debug collector. Payloads exclude tokens/secrets and only include status codes, route/url, map counts, and socket metadata.
