...
class Map extends Controller\AccessController {
    ...
    protected function updateMapByCharacter(Pathfinder\MapModel $map, Pathfinder\CharacterModel $character, array $newSystemPositions = []) : Pathfinder\MapModel {
        ...
        if(
            ...
            if($sourceSystemId){
                ...
                $systemOffsetX = 130;
                $systemOffsetY = 0;
                $systemPosX = ((int)($defaultPositions[0]['x'] ?? 0)) ? : 0;
                $systemPosY = ((int)($defaultPositions[0]['y'] ?? 0)) ? : 30;
                ...
                if($sourceSystemId === $targetSystemId){
                    ...
                    if($targetSystem){
                        ...
                        if($targetSystemId === (int)($currentPosition['systemId'] ?? 0)){
                            $systemPosX = (int)($currentPosition['position']['x'] ?? 0);
                            $systemPosY = (int)($currentPosition['position']['y'] ?? 0);
                        }
                    }else{
    ...
    protected function resolvePlacementPosition(
        ...
        // ⚠️ 힌트 좌표는 클라이언트의 (수 초 전) 맵 스냅샷 기준이라, 그 사이 다른 경로(데몬/타 유저)로
        // 시스템이 추가됐다면 이미 점유된 자리일 수 있다. 무검증 채택 시 새 시스템이 기존 노드
        // (예: 미리 열어둔 웜홀) 위에 정확히 겹쳐 올라가므로, 반드시 겹침 검사를 통과한 경우에만 쓰고
        // 아니면 source-anchor 탐색으로 폴백한다.
        ...
        if (
            ...
            $hintX = (int)$newSystemPositions['location']['position']['x'];
            $hintY = (int)$newSystemPositions['location']['position']['y'];

            if ($isPositionFree($hintX, $hintY)) {
                return [
                    'x' => $hintX,
                    'y' => $hintY,
            ...
            // 점유된 힌트는 버리고 source/target-anchor 탐색으로 폴백
        }
